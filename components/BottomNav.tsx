...
type Page = 'today'|'stats'|'settings'
type Props = { page: Page; onNav: (p: Page) => void; accent: string }

export default function BottomNav({ page, onNav, accent }: Props) {
  const btn = (p: Page, label: string, icon: React.ReactNode) => (
    <button onClick={() => onNav(p)}
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex z-50" style={{ paddingBottom: 'env(safe-area-inset-bottom,0px)' }}>
      {btn('today', 'Today',
        <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
      )}
      {btn('stats', 'Stats',
